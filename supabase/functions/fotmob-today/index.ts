...
function parseFotmobMatches(markdown: string): { topLeagues: League[]; otherLeagues: League[] } {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('#') || line.startsWith('**') || line.includes('League') || line.includes('Cup') || line.includes('Serie') || line.includes('Bundesliga')) {
    ...
    if (matchResult && currentLeague) {
      ...
      const matchId = `fm-${Date.now()}-${leagues.get(leagueId)!.matches.length}`;
...
Deno.serve(async (req) => {
  ...
  try {
    if (cache && Date.now() - cache.timestamp < CACHE_TTL) {
      console.log('Returning cached FotMob today matches');
    ...
    console.log('Scraping FotMob today matches...');
