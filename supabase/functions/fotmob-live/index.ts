...
function parseFotmobData(markdown: string): Match[] {
  const matches: Match[] = [];
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (line.startsWith('#') || line.startsWith('**') || line.includes('League') || line.includes('Cup') || line.includes('Serie') || line.includes('Bundesliga') || line.includes('La Liga') || line.includes('Premier')) {
    ...
    const livePattern = /(\d+)'|HT|FT|LIVE|Live|1H|2H|Half-time/i;
    ...
    if (matchResult) {
      ...
      if (timeMatch) {
        ...
        if (timeStr === 'HT' || timeStr.includes('HALF')) {
          status = 'HT';
          minute = 'HT';
        } else if (timeStr === 'FT') {
          status = 'FT';
      ...
      const matchId = `fm-${Date.now()}-${matches.length}`;
      ...
      matches.push({
        ...
        homeTeam: {
          ...
          logo: `https://ui-avatars.com/api/?name=${encodeURIComponent(homeTeam.trim())}&background=1e3a5f&color=3b82f6&bold=true&size=64`,
        },
        awayTeam: {
          id: awayTeam.trim().toLowerCase().replace(/\s/g, '-'),
          name: awayTeam.trim(),
          shortName: awayTeam.trim().substring(0, 3).toUpperCase(),
          logo: `https://ui-avatars.com/api/?name=${encodeURIComponent(awayTeam.trim())}&background=1e3a5f&color=3b82f6&bold=true&size=64`,
        },
...
Deno.serve(async (req) => {
  ...
  try {
    if (cache && Date.now() - cache.timestamp < CACHE_TTL) {
      console.log('Returning cached FotMob live matches');
    ...
    console.log('Scraping FotMob live matches...');
    ...
    console.log('Firecrawl response received from FotMob');
    
    const markdown = data.data?.markdown || data.markdown || '';
    const matches = parseFotmobData(markdown);
    
    const liveMatches = matches.filter(m => 
      m.status === 'LIVE' || m.status === 'HT' || m.status === '1H' || m.status === '2H'
    );
    
    console.log(`Parsed ${liveMatches.length} live matches from FotMob`);
    ...
    console.error('Error scraping FotMob:', error);
